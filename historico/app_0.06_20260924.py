...
PATH_FOCOS_PARQUET = "/home/lobs/SIG/FOCOS DE CALOR/bdqueimadas_consolidado.parquet"
...
def carregar_areas_protegidas(caminho_uc, caminho_ti):
    ...
    for tipo, caminho in (("UC", caminho_uc), ("TI", caminho_ti)):
        ...
        for feat in data.get("features", []):
            ...
            nome = str(feat.get("properties", {}).get("name", "")).strip() or "Sem nome"
...
def _build_pdf_kpis(kpis, styles):
    ...
    ano_ini, ano_fim = kpis["ano_inicio"], kpis["ano_fim"]
    ...
    kpi_data = [
        _rotulos(
            f"Focos em {ano_fim}",
            f"Posição no estado — acumulado {ano_ini}–{ano_fim}",
...
if btn_processar:
    ...
    with st.spinner("Processando geodados, sazonalidade e gerando mapas..."):
        ...
        total_muns_estado = len(muns_acum)
        ...
        kpis_dict = {
            ...
            "total_muns_estado": total_muns_estado,
