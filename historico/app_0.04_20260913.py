...
def generate_trimester_recurrence_map(
    ...
    for idx, (titulo_trim, meses) in enumerate(trimestres.items()):
        ...
        # Desenha o limite territorial do IBGE e obtém os limites geográficos (minx, maxx, miny, maxy)
        bounds = plot_polygon_ibge(ax, poligono_municipio)

        # Filtra as coordenadas pertencentes aos meses do trimestre atual
        pts_trim = [
...
def plot_polygon_ibge(
    ax, geom, color="#1E88E5", linewidth=1.5, fill_alpha=0.05
):
    ...
    # Captura os limites (minx, miny, maxx, maxy) e ajusta a visualização
...
def _build_pdf_tables_and_notes(df_est, df_reg, ano_fim, styles):
    ...
    # 1. Tabela Ranking Estadual (Inserida sequencialmente, sem aninhamento)
    elements.append(
    ...
    # 2. Tabela Ranking Regional (Inserida sequencialmente)
    elements.append(
        Paragraph(
...
# =========================================================================
# FUNÇÃO PRINCIPAL ORQUESTRADORA (REFATORADA)
# =========================================================================
def generate_pdf_report(
    ...
    # Montagem modular e limpa do relatório
...
def carregar_geojson_estado(caminho_arquivo: str) -> dict:
    ...
    print(
        f"[LOG] Carregando base geográfica do estado: {os.path.basename(caminho_arquivo)}"
    )
...
def obter_mapa_estados(diretorio_base: str) -> dict:
    ...
    mapa = {}
    for arq in arquivos:
        # Extrai o nome do estado a partir do padrao 'NM_UF_<Estado>.geojson'
        nome_estado = (
...
def extrair_dados_municipio(geojson_data: dict, municipio_nome: str):
    ...
    for feat in geojson_data.get("features", []):
        ...
        if props.get("NM_MUN", "").strip().upper() == municipio_nome.strip().upper():
            # Converte a geometria GeoJSON para um objeto interativo da biblioteca Shapely
...
# ----------------------------------------------------------------------
anos_disponiveis, _ = get_available_years_and_municipalities()

if not anos_disponiveis:
    st.error("Nenhum arquivo GeoJSON de focos encontrado no diretório de dados.")
    st.stop()

st.sidebar.title("🛠️ Parâmetros da Análise")

# --- SELEÇÃO DE ESTADO E MUNICÍPIO (BASE NACIONAL IBGE) ---
st.sidebar.subheader("Localização (IBGE)")
...
caminho_geojson_uf = mapa_estados[estado_selecionado]
geojson_uf = carregar_geojson_estado(caminho_geojson_uf)

# 2. Lista os municípios contidos no estado selecionado
lista_municipios = sorted([
    feat["properties"]["NM_MUN"]
    for feat in geojson_uf.get("features", [])
    if "properties" in feat and "NM_MUN" in feat["properties"]
])

# 3. Seleção do Município (Base IBGE)
municipio_selecionado = st.sidebar.selectbox(
    "📍 Selecione o Município:",
    options=lista_municipios
)

# 4. Extração dos metadados e do Polígono oficial do IBGE
props_ibge, poligono_municipio = extrair_dados_municipio(geojson_uf, municipio_selecionado)

if props_ibge:
    st.sidebar.caption(
        f"**Código IBGE:** {props_ibge.get('CD_MUN')}  \n"
        f"**Área:** {props_ibge.get('AREA_KM2')} km²  \n"
        f"**Região:** {props_ibge.get('NM_RGI')}"
    )
...
if btn_processar:
    ...
    with st.spinner("Processando geodados, sazonalidade e gerando mapas..."):
        ...
        # -------------------------------------------------------------------------
        print(
            "[ETAPA 4/6] Gerando mapas geospaciais com limites do IBGE..."
        )

        st.markdown("### 🗺️ Mapeamento Espacial e Sazonalidade Trimestral")

        # Chama a geração dos mapas usando o polígono do IBGE vindo do sidebar (poligono_municipio)
        ...
        st.divider()
        print(
            "[ETAPA 4/6 OK] Mapeamento concluído com limites territoriais IBGE."
        )
