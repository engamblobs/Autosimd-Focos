...
def create_proportional_image(buf, max_w=535, max_h=660):
    """Redimensiona a imagem para o limite máximo da página mantendo estritamente a proporção original."""
    buf.seek(0)
...
# ----------------------------------------------------------------------
def load_data_from_geojsons(data_dir=DATA_DIR, municipio_alvo="ALTAMIRA", uf="PA"):
    ...
    total_muns = len(ranking_acumulado)
    idx_alvo = pos_historico - 1

    inicio_idx = max(0, idx_alvo - 2)
    fim_idx = min(total_muns, inicio_idx + 5)

    if (fim_idx - inicio_idx) < 5:
        inicio_idx = max(0, fim_idx - 5)

    vizinhos_slice = ranking_acumulado[inicio_idx:fim_idx]

    proximos_muns = [f"{inicio_idx + i + 1}º {m[0].title()}" for i, m in enumerate(vizinhos_slice)]
    proximos_focos = [m[1] for m in vizinhos_slice]
    ...
    return {
        ...
        "ranking_proximo": {"municipios": proximos_muns, "focos": proximos_focos},
...
def generate_p1_charts(proximos_muns, proximos_focos, hist_anos, hist_focos, municipio_alvo):
    fig1, ax1 = plt.subplots(figsize=(3.5, 1.6), dpi=150)
    
    # Pinta em vermelho o município consultado e em azul claro os vizinhos
    bar_colors = ["#b52b27" if municipio_alvo.lower() in m.lower() else "#a6b1e1" for m in proximos_muns]

    ax1.barh(proximos_muns[::-1], proximos_focos[::-1], color=bar_colors[::-1])
    ax1.set_title("Municípios Próximos no Ranking Histórico", fontsize=8, fontweight="bold")
    ...
    ax2.plot(hist_anos, hist_focos, marker="o", color="#b52b27", linewidth=1.5, markersize=4)
    ax2.set_title(f"Evolução dos Focos em {municipio_alvo.title()}", fontsize=8, fontweight="bold")
...
# ----------------------------------------------------------------------
# Gerador de Mapas de Página Inteira (Centralizados + Grade Suave 10km)
# ----------------------------------------------------------------------
def generate_full_page_maps(coords_atual, coords_hist_ano, municipio, ano_fim, ano_inicio, kml_polygons):
    """Gera 3 mapas de página inteira centralizados com grade de 10km suave e escala customizada."""

    GRID_10KM_DEG = 10.0 / 111.0

    # Escala de cores estendida: Azul -> Verde -> Amarelo -> Laranja -> Vermelho -> Roxo
    ...
    fig1, ax1 = plt.subplots(figsize=(8.0, 9.5), dpi=220)
    ax1.set_facecolor("#F8F9FA")
    bounds = draw_kml_boundary(ax1, kml_polygons)

    if coords_atual:
        lons, lats = zip(*coords_atual)
        ax1.scatter(lons, lats, c="#b52b27", s=14, alpha=0.8, edgecolors="none", zorder=3, label="Foco de Calor")
        ax1.set_title(f"Dispersão Espacial dos Focos em {ano_fim} — {municipio} ({len(coords_atual):,} focos)".replace(",", "."), fontsize=10, fontweight="bold", pad=10)
    else:
        ax1.text(0.5, 0.5, f"Sem focos registrados em {ano_fim}", ha="center", va="center", fontsize=10, transform=ax1.transAxes)
        ax1.set_title(f"Dispersão Espacial dos Focos em {ano_fim}", fontsize=10, fontweight="bold")

    ax1.set_xlabel("Longitude", fontsize=8.5)
    ax1.set_ylabel("Latitude", fontsize=8.5)
    ...
    # 2. Densidade Espacial Acumulada (Grade 10km com bordas leves e paleta estendida)
    fig2, ax2 = plt.subplots(figsize=(8.0, 9.5), dpi=220)
    ...
    if coords_hist_ano and bounds:
        ...
        # Grid com bordas finas e translúcidas (edgecolors com alpha reduzido)
        ...
        ax2.set_title(f"Densidade Espacial Acumulada ({ano_inicio}-{ano_fim}) — Grade 10 km × 10 km — {municipio} ({len(coords_hist_ano):,} focos)".replace(",", "."), fontsize=10, fontweight="bold", pad=10)
    else:
        ax2.text(0.5, 0.5, "Sem dados históricos georreferenciados", ha="center", va="center", fontsize=10, transform=ax2.transAxes)
        ax2.set_title("Densidade Espacial Acumulada Histórica", fontsize=10, fontweight="bold")

    ax2.set_xlabel("Longitude", fontsize=8.5)
    ax2.set_ylabel("Latitude", fontsize=8.5)
    ...
    # 3. Recorrência Temporal (Grade 10km com bordas leves)
    fig3, ax3 = plt.subplots(figsize=(8.0, 6.2), dpi=220)
    ...
    if coords_hist_ano and bounds:
        ...
        years = sorted(list(set(ano for _, _, ano in coords_hist_ano)))
        rec_matrix = np.zeros((len(x_bins) - 1, len(y_bins) - 1))

        for y_val in years:
            pts_year = [(lon, lat) for lon, lat, a in coords_hist_ano if a == y_val]
            if pts_year:
                lons_y, lats_y = zip(*pts_year)
                counts_y, _, _ = np.histogram2d(lons_y, lats_y, bins=[x_bins, y_bins])
                rec_matrix += (counts_y > 0).astype(int)
        ...
        ax3.set_title(f"Recorrência Temporal dos Focos ({ano_inicio}-{ano_fim}) — Grade 10 km × 10 km", fontsize=10, fontweight="bold", pad=10)
    else:
        ax3.text(0.5, 0.5, "Sem dados suficientes para cálculo de recorrência", ha="center", va="center", fontsize=10, transform=ax3.transAxes)
        ax3.set_title("Recorrência Espacial dos Focos", fontsize=10, fontweight="bold")

    ax3.set_xlabel("Longitude", fontsize=8.5)
    ax3.set_ylabel("Latitude", fontsize=8.5)
    ...
    img1 = create_proportional_image(buf1, max_w=535, max_h=660)
    img2 = create_proportional_image(buf2, max_w=535, max_h=660)
    img3 = create_proportional_image(buf3, max_w=535, max_h=450)

    return img1, img2, img3


def build_centered_table(image_obj):
    """Centraliza o mapa horizontalmente e verticalmente na página."""
    t = Table([[image_obj]], colWidths=[535])
...
# ----------------------------------------------------------------------
# Construtor do PDF (5 Páginas)
# ----------------------------------------------------------------------
def generate_full_report(dados, output_filename="relatorio_completo_altamira.pdf"):
    ...
    tbl_header_style = ParagraphStyle(
        "TblHeader", fontName="Helvetica-Bold", fontSize=7, leading=8.5, textColor=colors.white
    )
    tbl_cell_style = ParagraphStyle(
        "TblCell", fontName="Helvetica", fontSize=7, leading=8.5, textColor=colors.HexColor("#222222")
    )
    ...
    c1, c2 = generate_p1_charts(
        dados["ranking_proximo"]["municipios"],
        dados["ranking_proximo"]["focos"],
    ...
    # Gerar mapas em escala inteira
    map1, map2, map3 = generate_full_page_maps(
    ...
    # ==================================================================
    # PÁGINA 2: MAPA 1 - DISPERSÃO ESPACIAL
    # ==================================================================
    elements.append(PageBreak())
    elements.append(Paragraph(f"1. DISPERSÃO ESPACIAL DOS FOCOS DE CALOR — {dados['municipio'].upper()}", meth_title_style))
    elements.append(Paragraph(
        f"Distribuição georreferenciada de todas as ocorrências detectadas no ano de {dados['ano_fim']} "
        "sobre o polígono territorial oficial do município.",
        meth_text_style
    ))
    elements.append(Spacer(1, 4))
    ...
    # ==================================================================
    # PÁGINA 3: MAPA 2 - DENSIDADE ESPACIAL ACUMULADA (10KM X 10KM)
    # ==================================================================
    elements.append(PageBreak())
    elements.append(Paragraph(f"2. DENSIDADE ESPACIAL ACUMULADA — {dados['municipio'].upper()}", meth_title_style))
    elements.append(Paragraph(
        f"Concentração e adensamento histórico de queimadas e incêndios florestais ({dados['ano_inicio']}-{dados['ano_fim']}) "
        "agrupados em grade regular de 10 km × 10 km.",
        meth_text_style
    ))
    elements.append(Spacer(1, 4))
    ...
    # ==================================================================
    # PÁGINA 4: MAPA 3 - RECORRÊNCIA TEMPORAL (10KM X 10KM) + SÍNTESE
    # ==================================================================
    elements.append(PageBreak())
    elements.append(Paragraph(f"3. RECORRÊNCIA TEMPORAL DOS FOCOS — {dados['municipio'].upper()}", meth_title_style))
    elements.append(Paragraph(
        "Mapeamento de reincidência multianual agregando as ocorrências em grade regular de 10 km × 10 km.",
        meth_text_style
    ))
    ...
    spatial_summary = [
        ...
        [
            ...
            Paragraph("Grade regular de 10 km × 10 km (~0.09°) para medição da reincidência anual acumulada.", tbl_cell_style),
        ],
        [
            Paragraph("Aplicação Defesa Civil", tbl_cell_style),
            Paragraph("Direcionamento de sobrevoos, patrulhas e instalação de bases avançadas de combate.", tbl_cell_style),
        ]
    ]

    spatial_table = Table(spatial_summary, colWidths=[150, 385])
    ...
    # ==================================================================
    # PÁGINA 5: METODOLOGIA E CANAIS DE SUPORTE
    # ==================================================================
    elements.append(PageBreak())
    ...
    elements.append(Paragraph(
        ...
        "• <b>Mapa de Densidade Espacial:</b> Agrupamento em grade regular de 10 km × 10 km (~0.09°) com paleta cromática ampliada.<br/>"
        "• <b>Mapa de Recorrência Espacial:</b> Avalia a frequência multianual dividindo o território em células de 10 km × 10 km. "
    ...
    elements.append(Paragraph(
        "Os dados deste relatório foram processados automaticamente por algoritmos desenvolvidos "
        "<b>com apoio de Inteligência Artificial orientada por técnicos de Defesa Civil</b>, garantindo precisão técnica e agilidade analítica.<br/><br/>"
...
if __name__ == "__main__":
    ...
    dados_reais = load_data_from_geojsons(data_dir=DATA_DIR, municipio_alvo="ALTAMIRA")
    ...
    generate_full_report(dados_reais, output_filename="relatorio_completo_altamira.pdf")
